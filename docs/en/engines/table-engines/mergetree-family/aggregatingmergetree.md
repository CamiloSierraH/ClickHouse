...
First we will create the table that contain the raw data, the table `test.visits`:
...
`AggregatingMergeTree` materialized view that watches the `test.visits` table, for this exmaple we use the `AggregateFunction` type:
